...
def visualize(model, data_loader, name, num_batches=1600):
    validation_outputs=[]
...
def main():
    ...
    model.eval()
    # visualize(model, val_dataloader, name=f"viz_out/viz_val_{pathlib.Path(args.ckpt_path).stem}", num_batches=1000)
    visualize(model, train_dataloader, name=f"viz_out/viz_train_{pathlib.Path(args.ckpt_path).stem}", num_batches=31) 
