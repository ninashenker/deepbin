...
class BertBin(pl.LightningModule):
    ...
    def __init__(self, kmer_dataset, val_dataset):
        ...
        self._train_dataloader = DataLoader(kmer_dataset, batch_size=16, shuffle=True, num_workers=30, drop_last=True)
        self._val_dataloader = DataLoader(val_dataset, batch_size=2, shuffle=False, num_workers=30, drop_last=False)
    ...
    def training_step(self, batch, batch_idx):
        ...
        return {'loss': train_loss.cpu(), 'taxonomy': taxonomy_labels, 'sample_labels': sample_labels}
    ...
    def validation_step(self, batch, batch_idx):
        ...
        cls = torch.flatten(last_hidden_state, 1)
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        plt.savefig('flatten_epoch{nr}.png'.format(nr = self.current_epoch))
        plt.clf()
...
class GenomeKmerDataset(torch.utils.data.Dataset):
    def __init__(self, contigs, k=4, genomes=10):
        ...
        tuple_cache_file = '/mnt/data/CAMI/DNABERT/val_tuples.pickle'
        if os.path.exists(tuple_cache_file):
            with open(tuple_cache_file, 'rb') as fp:
        ...
        #ATTN: will not work if you change the sample
        ...
        i = 0
        genome_dict = dict()
        groups = list(merged_df_sample_6.groupby(GROUP_KEY))
...
def main():
    ...
    checkpoint_callback = ModelCheckpoint(
            dirpath='/mnt/data/CAMI/DNABERT/flatten_models',
